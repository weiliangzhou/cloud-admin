package com.zwl.common.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.zwl.common.config.MsgCode;
import com.zwl.common.config.MsgSenderConstants;
import com.zwl.common.exception.BSUtil;
import com.zwl.common.service.MsgSenderService;
import com.zwl.common.utils.HttpsUtils;
...
public class MsgSenderServiceImpl implements MsgSenderService {
    ...
    @Override
    public R sendMsg(String phone, String msg) {
        try {
            Map map = new HashMap();
            map.put("account", MsgSenderConstants.UN);
            map.put("password", MsgSenderConstants.PW);
            ...
            if ("0".equals(code)) {
                ...
                return R.error(1,"短信发送失败!" + "错误原因:" + errorMsg);
            }
        } catch (Exception e) {
            log.error("发送短信出现未知错误", e);
            return R.error(2,"发送短信出现未知错误");
        }
